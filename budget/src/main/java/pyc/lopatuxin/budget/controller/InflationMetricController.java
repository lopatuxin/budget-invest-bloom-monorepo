...
import pyc.lopatuxin.budget.service.InflationMetricService;

/**
 * Контроллер для получения детальной метрики личной инфляции пользователя за год.
 */
@Slf4j
@RestController
@RequestMapping("/api/budget/metric/inflation")
@RequiredArgsConstructor
@Tag(name = "Метрики", description = "API для получения детальных метрик бюджета")
public class InflationMetricController {

    private final InflationMetricService inflationMetricService;

    /**
     * Возвращает детальную метрику личной инфляции пользователя за указанный год.
     *
     * @param request запрос с контекстом пользователя и годом
     * @return стандартный ответ с метрикой инфляции
     */
    @PostMapping
    @ResponseStatus(HttpStatus.OK)
    @Operation(
            summary = "Получить метрику личной инфляции",
            description = "Возвращает помесячную разбивку личной инфляции за указанный год, " +
                    "а также агрегированные показатели: текущее значение, среднее, максимум и процент изменения."
    )
    @ApiResponse(
            responseCode = "200",
            description = "Метрика инфляции успешно получена",
            content = @Content(
    ...
    public ResponseApi<MetricResponseDto> getInflationMetric(
        ...
        MetricResponseDto metric = inflationMetricService.getInflationMetric(
                request.getUser().getUserId(),
                request.getData().getYear()
        );
        return ResponseApi.success("Метрика инфляции успешно получена", metric);
    }
}
