...
class IncomeMetricServiceUnitTest {
    ...
    void shouldReturnCorrectMetricWhenSeveralMonthsHaveData() {
        ...
        IncomeMetricResponseDto result = incomeMetricService.getIncomeMetric(userId, year);
    ...
    void shouldReturnZeroValuesWhenNoDataExists() {
        ...
        for (MonthlyIncomeDto monthly : result.getMonthlyData()) {
    ...
    void shouldPopulateAllMonthNamesCorrectly() {
        ...
        for (int i = 0; i < 12; i++) {
            MonthlyIncomeDto monthly = result.getMonthlyData().get(i);
